...
const GameCard: React.FC<GameCardProps> = ({ game, onPlay, onEdit, onDelete, currentUserId }) => {
  ...
  const getIcon = () => {
    switch(game.gameType) {
      ...
      default: return <Brain className="h-8 w-8 text-zinc-400" />;
    }
  };
  ...
  return (
    <div className="card-hover bg-zinc-900 rounded-xl shadow-lg border border-zinc-800 overflow-hidden flex flex-col h-full group relative hover:border-zinc-600 transition-colors">
      ...
      {isOwner && (
          <div className="absolute top-2 right-2 flex space-x-1 opacity-0 group-hover:opacity-100 transition-opacity z-20">
            <button 
              ...
              className="p-2 bg-black/80 backdrop-blur rounded-full hover:bg-white hover:text-black text-white shadow-lg border border-zinc-700 transition-colors"
              title="Düzenle"
            >
            ...
            <button 
              ...
              className="p-2 bg-black/80 backdrop-blur rounded-full hover:bg-red-600 text-white shadow-lg border border-zinc-700 transition-colors"
              title="Sil"
            >
      ...
      {isOwner && (
         <div className="absolute top-2 left-2 z-10">
             <span className={`flex items-center text-[10px] uppercase font-bold px-2 py-1 rounded-full border ${game.isPublic ? 'bg-black/50 text-emerald-400 border-emerald-900' : 'bg-black/50 text-zinc-400 border-zinc-800'}`}>
      ...
      <div className="h-32 w-full flex items-center justify-center bg-black/40 group-hover:bg-black/60 transition-colors cursor-pointer border-b border-zinc-800" onClick={() => onPlay(game)}>
        {getIcon()}
      </div>
      
      <div className="p-5 flex-grow flex flex-col cursor-pointer" onClick={() => onPlay(game)}>
        <div className="flex items-center justify-between mb-3">
           <span className="text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 bg-zinc-800 rounded text-zinc-400">
             {getLabel()}
           </span>
        </div>
        <h3 className="font-bold text-white mb-2 line-clamp-1 text-lg leading-snug break-words">{game.title}</h3>
        <p className="text-sm text-zinc-400 line-clamp-2 break-words leading-relaxed">{game.description}</p>
        <div className="mt-auto pt-4 flex items-center justify-between border-t border-zinc-800/50">
            <span className="text-xs text-zinc-500 truncate max-w-[100px]">{game.author}</span>
            <span className="text-xs text-white flex items-center font-semibold bg-zinc-800 px-2 py-1 rounded-full">
